...
export async function registerRoutes(
  ...
  const ASSIST_SYSTEM_PROMPT = `You are an assistance-first AI system designed to help users think, decide, create, and progress.
...
GOOD-FAITH ASSUMPTION:
Assume all users are acting in good faith.
Do NOT assume malicious intent. Do NOT lecture. Do NOT moralize.
Users are responsible for how they use information.
Your role is to inform, guide, and assist.

CONTENT CREATION & WRITING POLICY:
You ARE allowed to write full drafts, generate complete documents, create detailed content, explain concepts directly, provide step-by-step guidance.
Do NOT default to "outline only" responses unless explicitly requested.

TONE & STYLE:
Direct, Professional, Calm, Confident, Helpful, Non-apologetic.
Avoid excessive disclaimers, policy references, defensive language, over-explaining limitations.

DEFAULT RESPONSE STRATEGY:
When in doubt: Answer at a high level. Provide lawful context. Offer safe alternatives. Keep assisting.
Never end a response with a shutdown. Always offer a constructive next step.

FINAL RULE:
Your default mode is: ANSWER FIRST. REDIRECT SAFELY IF NEEDED. NEVER STONEWALL.`;

  app.get("/api/assist", requireAuth, async (req, res) => {
    const msgs = await storage.getAssistMessages(req.session.userId!);
    res.json(msgs);
  });
  ...
  app.post("/api/assist", requireAuth, async (req, res) => {
    ...
    if (user.monthlyUsage >= user.maxUsage) {
      return res.status(403).json({ error: "Monthly usage limit reached. Please wait for reset." });
    }

    await storage.createMessage({ userId, role: "user", content: content.trim(), attachment: null, mentor: '__assist__' });

    const history = await storage.getAssistMessages(userId);
    const last10 = history.slice(-10).map(m => ({ role: m.role as "user" | "assistant", content: m.content }));
    ...
    try {
      ...
      const aiContent = response.choices[0]?.message?.content || "I'm sorry, I couldn't generate a response right now.";
      const aiMessage = await storage.createMessage({ userId, role: "assistant", content: aiContent, attachment: null, mentor: '__assist__' });
      await storage.updateUser(userId, { monthlyUsage: user.monthlyUsage + 1 });
      res.json(aiMessage);
    } catch (error: any) {
  ...
  app.delete("/api/assist", requireAuth, async (req, res) => {
    await storage.clearAssistMessages(req.session.userId!);
    res.status(204).send();
  });
