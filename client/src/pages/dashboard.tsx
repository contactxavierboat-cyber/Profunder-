...
export default function DashboardPage() {
  ...
  useEffect(() => {
    if (activeTab === "discover" && feedItems.length === 0) {
      fetchFeed();
    }
  }, [activeTab]);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => fetchFeed(false), 3 * 60 * 1000);
          ...
          <div className="h-14 flex items-center justify-between px-4">
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-[15px] font-bold tracking-tight">MentXr®</span>
              </div>
            </div>
          ...
          <div className="flex">
            <button
              ...
              className={cn(
                ...
                activeTab === "chat" ? "text-white" : "text-white/35 hover:text-white/55"
              )}
            >
              ...
              {activeTab === "chat" && <div className="absolute bottom-0 left-1/4 right-1/4 h-[2px] bg-[#E0E0E0] rounded-full" />}
            </button>
            <button
              ...
              className={cn(
                "flex-1 py-2.5 text-[13px] font-semibold text-center transition-colors relative",
                activeTab === "discover" ? "text-white" : "text-white/35 hover:text-white/55"
              )}
            >
              <span className="flex items-center justify-center gap-1.5">
              ...
              {activeTab === "discover" && <div className="absolute bottom-0 left-1/4 right-1/4 h-[2px] bg-[#E0E0E0] rounded-full" />}
            </button>
          </div>
        </header>
          ...
          {activeTab === "discover" ? (
            <div className="max-w-xl mx-auto w-full px-4 py-4">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  <Globe className="w-4 h-4 text-[#E0E0E0]/60" />
                  <span className="text-[14px] font-semibold text-white/80">Trending Now</span>
                </div>
                <button
                  onClick={() => fetchFeed(true)}
                  disabled={feedLoading}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white/[0.04] border border-white/[0.08] text-[11px] text-white/40 hover:text-white/60 hover:bg-white/[0.08] transition-colors"
              ...
              {feedLoading && feedItems.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20">
                  ...
                  <p className="text-[13px] text-white/30">Loading live feed...</p>
                </div>
              ) : feedItems.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20">
                  <Globe className="w-8 h-8 text-white/10 mb-3" />
                  <p className="text-[13px] text-white/30">No feed items available right now</p>
                  <button onClick={() => fetchFeed(true)} className="mt-3 text-[12px] text-[#E0E0E0]/50 hover:text-[#E0E0E0] transition-colors">Try again</button>
                </div>
              ) : (
                <div className="space-y-3">
                  {feedItems.map((item: any, idx: number) => (
                    ...
                    >
                      {item.image && (
                        <div className="w-full h-40 sm:h-48 bg-[#111] overflow-hidden">
                          <img
                            src={item.image}
                            alt={item.title}
                      ...
                      <div className="p-4">
                        <div className="flex items-center gap-2 mb-2">
                          <span className="px-2 py-0.5 rounded-full bg-white/[0.06] text-[10px] font-medium text-white/40 uppercase tracking-wide">{item.category}</span>
                        ...
                        <h3 className="text-[15px] font-semibold text-white/90 leading-snug mb-1.5 group-hover:text-white transition-colors">{item.title}</h3>
                        {item.description && (
                          <p className="text-[13px] text-white/45 leading-relaxed line-clamp-2">{item.description}</p>
                        )}
                        <div className="flex items-center gap-1.5 mt-3 text-[11px] text-white/25">
              ...
              {feedLastUpdated && (
                <p className="text-center text-[10px] text-white/15 mt-6">
                  Last updated {timeAgo(feedLastUpdated)} · Auto-refreshes every 3 min
                </p>
              )}
            </div>
          ) : !hasMessages ? (
