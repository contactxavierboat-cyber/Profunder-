    ...
    <div className="min-h-[100dvh] bg-white text-[#1A1A1A] flex">
      ...
      <aside className={cn(
        ...
        <div className="p-4 sm:p-6 border-b border-[#E5E7EB] flex items-center justify-between">
          <div>
            <h1 className="font-sans font-bold text-lg sm:text-xl tracking-tighter flex items-center gap-2 text-[#1A1A1A]">
              <span className="relative w-7 h-7 sm:w-8 sm:h-8 flex items-center justify-center">
                ...
                <span className="relative w-3 h-3 sm:w-3.5 sm:h-3.5 rounded-full bg-[#2E7D32] shadow-[0_0_8px_rgba(46,125,50,0.4)]" />
              </span>
              MentXr®
            </h1>
            <p className="text-[10px] sm:text-xs text-[#666] mt-1">Mentorship, On Demand</p>
          </div>
          <button
            className="md:hidden w-8 h-8 flex items-center justify-center rounded-lg hover:bg-[#EEF2F4]"
            onClick={() => setMobileOpen(false)}
          >
        ...
        <nav className="flex-1 p-3 sm:p-4 space-y-1 sm:space-y-2">
          {navItems.map((item) => (
            <Link key={item.href} href={item.href} onClick={() => setMobileOpen(false)} className={cn(
              ...
              location === item.href 
                ? "bg-[#EEF2F4] text-[#2E7D32] font-semibold" 
                : "text-[#333] hover:bg-[#EEF2F4] hover:text-[#1A1A1A]"
            )}>
        ...
        <div className="p-3 sm:p-4 border-t border-[#E5E7EB]">
          <div className="flex items-center gap-3 px-3 py-2 sm:py-3 mb-2">
            <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-[#EEF2F4] flex items-center justify-center text-[10px] sm:text-xs font-mono text-[#333]">
              {(user.displayName || user.email).substring(0, 2).toUpperCase()}
            </div>
            <div className="overflow-hidden">
              <p className="text-sm font-medium truncate text-[#1A1A1A]">{user.displayName || user.email}</p>
              <p className="text-xs text-[#666] capitalize">{user.role}</p>
            </div>
          </div>
          <Button variant="outline" className="w-full justify-start text-[#666] border-[#E5E7EB] hover:bg-[#EEF2F4] hover:text-[#1A1A1A]" onClick={logout}>
      ...
      <div className="flex-1 md:ml-64 bg-white relative flex flex-col min-h-[100dvh]">
        <header className="h-12 flex items-center justify-between px-3 border-b border-[#E5E7EB] md:hidden shrink-0 relative z-10 bg-white">
          <button
            ...
            className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-[#EEF2F4]"
          >
            <Menu className="w-5 h-5 text-[#666]" />
          </button>
          <h1 className="text-sm font-bold text-[#1A1A1A]">MentXr®</h1>
          <div className="w-8" />
        </header>
